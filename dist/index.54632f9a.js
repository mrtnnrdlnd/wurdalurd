...
var _wordsLengthFiveLong = require("./wordsLengthFiveLong");
let wordListFiltered = (0, _wordsLengthFiveLong.wordsLengthFive);
// let ratedWordListFiltered = wordList.map(word => ({word: word, rating: 0} as RatedWord))
function cycleFilter(currentFilter) {
    const keys = Object.keys(FilterType);
    return FilterType[(keys.indexOf(currentFilter.toString()) + 1) % (keys.length / 2)];
}
var FilterType;
(function(FilterType) {
    FilterType[FilterType["NO_FILTER"] = 0] = "NO_FILTER";
    FilterType[FilterType["WRONG_POSITION"] = 1] = "WRONG_POSITION";
    FilterType[FilterType["RIGHT_POSITION"] = 2] = "RIGHT_POSITION";
    FilterType[FilterType["NOT_IN_WORD"] = 3] = "NOT_IN_WORD";
})(FilterType || (FilterType = {}));
function Filter(filterType) {
    const filterFunction = function(word, letter, position) {
        switch(filterType){
            case "NOT_IN_WORD":
...
function createLetterRow(length) {
    const element = document.createElement("div");
    element.className = "letter-row";
    return element;
}
function createLetterBox(letter, type) {
    const element = document.createElement(type);
    element.className = "letter-box";
    element.innerHTML = letter;
    return element;
}
let inputDiv = document.getElementById("input") ?? document.createElement("div");
for(let i = 0; i < 6; i++){
    const letterRow = createLetterRow(5);
    for(let j = 0; j < 5; j++){
        const letterBox = createLetterBox(" ", "input");
        letterBox.dataset.filter = FilterType[0];
        ...
        letterBox.onclick = (event)=>{
            const target = event.target;
            target.dataset.filter = cycleFilter(target.dataset.filter ?? FilterType[0]);
            updateGuesses(target);
            filterWordList();
        };
...
function updateGuesses(letterBox) {
    if (letterBox.dataset.position) {
        ...
        guesses[position].filter = letterBox.dataset.filter ?? FilterType[0];
    }
}
function updateShownWordList() {
    let outputDiv = document.getElementById("output") ?? document.createElement("div");
...
function filterWordList() {
    wordListFiltered = (0, _wordsLengthFiveLong.wordsLengthFive);
...
},{"./wordsLengthFiveLong":"eMny9"}],"eMny9":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "wordsLengthFive", ()=>wordsLengthFive);
...
},{}]},["2ralK","gg0zR"], "gg0zR", "parcelRequire716c")
